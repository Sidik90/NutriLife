...
async def get_due_reminders():
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        cursor = await db.execute("SELECT user_id, reminder_text, reminder_time FROM reminders WHERE is_sent = 0")
        ...
        for reminder in reminders:
            ...
            try:
                ...
                if reminder_time and reminder_time <= current_time:
                    ...
                    print(f"Напоминание добавлено в очередь на отправку: {reminder_text} на {reminder_time_str}")
                elif reminder_time:
                    print(f"Напоминание еще не наступило: {reminder_text} на {reminder_time_str}")
                else:
                    print(f"Не удалось распознать формат времени для напоминания: {reminder_text} на {reminder_time_str}")
            except Exception as e:
                print(f"Ошибка при обработке напоминания {reminder_text} на {reminder_time_str}: {str(e)}")
...
async def clean_old_reminders(days=7):
    ...
    async with aiosqlite.connect(DATABASE_FILE) as db:
        threshold_time = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
