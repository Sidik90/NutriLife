...
from keyboards.inline import get_yes_no_keyboard, get_diagnostic_result_keyboard
...
# Обработчик выбора темы диагностики
...
async def process_diagnostic_theme(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        if os.path.exists(video_path):
            await callback_query.message.answer_video(video=FSInputFile(video_path),
                                                      caption=f"Видео для темы {theme.capitalize()}")
            logger.info(f"Видео для темы {theme} успешно отправлено пользователю {user_id}")
        else:
            await callback_query.message.answer(
                f"Видео для темы {theme.capitalize()} не найдено. Начинаем диагностику.")
    ...
    questions = diagnostics_data[theme]["questions"]
    await state.update_data(theme=theme, question_index=0, questions=questions, answers=[])

    # Задаем первый вопрос
    first_question = questions[0]
    await callback_query.message.answer(f"Вопрос {first_question['id']}: {first_question['text']}",
                                        reply_markup=get_yes_no_keyboard(theme, first_question['id']))
...
async def process_diagnostic_answer(callback_query: CallbackQuery, state: FSMContext):
    ...
    answers.append({"question_id": current_question["id"], "answer": answer})
    logger.info(f"Пользователь {user_id} ответил на вопрос {current_question['id']} темы {theme}: {answer}")

    # Отправляем обратную связь
    await callback_query.message.edit_text("Спасибо за ответ! ✅")
    ...
    if question_index < len(questions):
        ...
        await callback_query.message.bot.send_message(chat_id=user_id,
                                                      text=f"Вопрос {next_question['id']}: {next_question['text']}",
                                                      reply_markup=get_yes_no_keyboard(theme, next_question['id']))
    else:
        # Завершение диагностики
        end_video_path = f"data/videos/{theme}_end.mp4"
        try:
            if os.path.exists(end_video_path):
                await callback_query.message.bot.send_video(chat_id=user_id, video=FSInputFile(end_video_path),
                                                            caption=f"Завершение диагностики по теме {theme.capitalize()}")
                logger.info(f"Завершающее видео для темы {theme} успешно отправлено пользователю {user_id}")
            else:
                await callback_query.message.bot.send_message(chat_id=user_id,
                                                              text=f"Завершающее видео для темы {theme.capitalize()} не найдено.")
                logger.warning(f"Завершающее видео для темы {theme} не найдено для пользователя {user_id}")
        except Exception as e:
            await callback_query.message.bot.send_message(chat_id=user_id,
                                                          text="Произошла ошибка при отправке завершающего видео.")
        ...
        await callback_query.message.bot.send_message(chat_id=user_id,
                                                      text="Диагностика завершена! Спасибо за ваши ответы. Вы можете записаться на консультацию или вернуться в главное меню.",
...
async def process_consultation(callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "Для записи на консультацию свяжитесь с нами через раздел 'Связаться с нами'.", reply_markup=get_main_menu())
    await callback_query.answer()