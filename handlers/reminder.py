...
async def start_reminder(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        "Введите текст напоминания:", reply_markup=get_cancel_keyboard()
    )
...
async def cancel_reminder(callback_query: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback_query.message.edit_text(
        "Создание напоминания отменено.", reply_markup=get_main_menu()
    )
    await callback_query.answer()
...
async def cancel_reminder_time(callback_query: CallbackQuery, state: FSMContext):
    user_id = callback_query.from_user.id
    logger.info(
        f"Пользователь {user_id} отменил создание напоминания на этапе ввода времени"
    )
    await state.clear()
    await callback_query.message.edit_text(
        "Создание напоминания отменено.", reply_markup=get_main_menu()
...
async def process_reminder_time(message: Message, state: FSMContext):
    ...
    try:
        ...
        reminder_time = datetime.strptime(
            reminder_time_input, "%d.%m.%Y %H:%M"
        ).strftime("%Y-%m-%d %H:%M")
        await save_reminder(user_id, reminder_text, reminder_time)
        await message.answer(
            f"Напоминание '{reminder_text}' установлено на {reminder_time_input} ⏰",
            reply_markup=get_main_menu(),
        )
        logger.info(f"Пользователь {user_id} установил напоминание на {reminder_time}")
    except ValueError:
        await message.answer(
            "Ошибка: неверный формат даты и времени. Используйте формат ДД.ММ.ГГГГ ЧЧ:ММ (например, 20.10.2023 15:30). Попробуйте снова.",
            reply_markup=get_cancel_keyboard(),
        )
        logger.warning(
            f"Пользователь {user_id} ввел неверный формат времени: {reminder_time_input}"
        )
    except Exception as e:
        await message.answer(
            "Произошла ошибка при сохранении напоминания. Попробуйте снова.",
            reply_markup=get_cancel_keyboard(),
        )
        logger.error(
            f"Ошибка при сохранении напоминания для пользователя {user_id}: {str(e)}"
        )
    finally:
        await state.clear()
...
async def check_reminders_command(message: Message):
    ...
    logger.info(
        f"Пользователь {user_id} запросил список напоминаний: найдено {len(user_reminders)} напоминаний"
    )
...
async def clear_invalid_reminders(message: Message):
    ...
    async with aiosqlite.connect("nutrilife.db") as db:
        cursor = await db.execute(
            "SELECT reminder_text, reminder_time FROM reminders WHERE user_id = ? AND is_sent = 0",
            (user_id,),
        )
        ...
        for reminder_text, reminder_time_str in reminders:
            try:
                ...
                await db.execute(
                    ...
                    (user_id, reminder_time_str),
                )
                invalid_count += 1
        await db.commit()
    await message.answer(
        f"Удалено {invalid_count} напоминаний с некорректным форматом времени."
    )
    logger.info(
        f"Пользователь {user_id} удалил {invalid_count} некорректных напоминаний"
    )
...
async def force_send_reminders(message: Message, bot):
    ...
    async with aiosqlite.connect("nutrilife.db") as db:
        cursor = await db.execute(
            "SELECT user_id, reminder_text, reminder_time FROM reminders WHERE is_sent = 0 AND user_id = ?",
            (user_id,),
        )
        reminders = await cursor.fetchall()
        sent_count = 0
            ...
            try:
                ...
                if reminder_time and reminder_time <= current_time:
                    await bot.send_message(
                        chat_id=user_id,
                        text=f"Напоминание (принудительная отправка): {reminder_text} ⏰",
                    )
                    await db.execute(
                ...
                logger.error(
                    f"Ошибка при принудительной отправке напоминания {reminder_text} на {reminder_time_str}: {str(e)}"
                )
        await db.commit()
    await message.answer(f"Принудительно отправлено {sent_count} напоминаний.")
    logger.info(
        f"Пользователь {user_id} принудительно отправил {sent_count} напоминаний"
    )
